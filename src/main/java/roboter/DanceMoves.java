...
class DanceMoves {
    ...
    static final DanceMove MOVE_RIGHT_MOTOR_20 = (robot, speed) -> {
        robot.rotateLeftMotor(speed, RotateAmount.rotations(-0.15F - 0.5F));
        robot.rotateRightMotor(speed.mult(2), RotateAmount.rotations(8 - 0.6F));
        robot.rotateLeftMotor(speed.negate(), RotateAmount.rotations(-0.15F - 0.5F));
    };
    static final DanceMove MOVE_LEFT_MOTOR_20 = (robot, speed) -> {
        robot.rotateRightMotor(speed, RotateAmount.rotations(-0.15F - 0.5F));
        robot.rotateLeftMotor(speed.mult(2), RotateAmount.rotations(8 - 0.6F));
        robot.rotateRightMotor(speed.negate(), RotateAmount.rotations(-0.15F - 0.5F));
    };
